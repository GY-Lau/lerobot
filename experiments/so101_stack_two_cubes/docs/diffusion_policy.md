...
[`jetson_diffusion_latency.csv`](../results/jetson_diffusion_latency.csv). Recompute a
single raw log with:
...
```bash
bash experiments/so101_stack_two_cubes/scripts/train_diffusion.sh --dry-run \
...
```bash
bash experiments/so101_stack_two_cubes/scripts/train_diffusion.sh \
  diffusion_stack_two_cubes_30k 30000 2 true 5000
```
