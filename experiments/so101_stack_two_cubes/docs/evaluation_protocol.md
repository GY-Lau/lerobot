...
```bash
python experiments/so101_stack_two_cubes/scripts/check_start_pose.py --profile relaxed
```
...
```bash
bash experiments/so101_stack_two_cubes/scripts/run_act_trial.sh \
...
```bash
python experiments/so101_stack_two_cubes/scripts/analyze_cube_placements.py \
  --dataset-root ~/.cache/huggingface/lerobot/GY-William/lerobot_stack_two_cubes \
  --output experiments/so101_stack_two_cubes/manifests/training_start_positions.csv
```
...
```bash
python experiments/so101_stack_two_cubes/scripts/log_trial.py \
...
```bash
python experiments/so101_stack_two_cubes/scripts/summarize_trials.py
```
